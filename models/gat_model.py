...
from core.layers import GAT, MLP, MinCostFlow, DualFlow, SparseMax
    ...
    def build(self, **kwargs):
        ...
        with self._sess.graph.as_default():
            with tf.variable_scope(self.name, reuse=tf.AUTO_REUSE):
                ...
                # Mask for non-neighbors during attention layer
                node_bias = -BIG_NUMBER * (1.0 - (adj + tf.eye(tf.shape(adj)[1])))

                node_gat = GAT(output_size=self.params['node_encoding'],
                ...
                bias = node_bias - BIG_NUMBER * tf.eye(tf.shape(node_bias)[1])
